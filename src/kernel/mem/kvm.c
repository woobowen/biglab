...
//
// 修复：将 extern char trampoline[]; 从 kvm_init 内部移到这里
//
...
void kvm_init()
{
    ...
    //
    // 修复：删除了这里的局部 "extern char trampoline[];" 声明
    //
...
//
// 修复：添加 kvm_map_trampoline 函数的定义
// (这个函数用于修复上一个 "undefined reference" 链接错误)
//
void kvm_map_trampoline(pgtbl_t pgtbl)
{
    // 为传入的 pgtbl 映射 TRAMPOLINE 页面
    // 权限为 R-X (可读、可执行)，不需要 U (用户态访问)
    //
